...
def AddTgraphBox(op_dict, step_small=0.1, step_big=0.1):
    ...
    for op in op_dict.keys():
        ...
        for process in op_dict[op].keys():
            ...
            g1 = ROOT.TGraphAsymmErrors(1, array('d', [0]), array('d', [y]), array('d', [abs(os[0])]), array('d', [abs(os[1])]), array('d', [0.03]), array('d', [0.03]))
            g1.SetLineWidth(1)
            g1.SetLineStyle(1)
            g1.SetMarkerStyle(0)
            g1.SetMarkerSize(0)
            #g1.SetFillStyle(1)
            g2 = ROOT.TGraphAsymmErrors(1, array('d', [0]), array('d', [y]), array('d', [abs(ts[0])]), array('d', [abs(ts[1])]), array('d', [0.03]), array('d', [0.03]))
            g2.SetLineWidth(1)
            g2.SetLineStyle(1)
            g2.SetMarkerStyle(0)
            g2.SetMarkerSize(0)
...
if __name__ == "__main__":
    ...
    plotting_nq["ZZ"] = {
        "file_" : "/eos/user/g/gboldrin/www/New/linearProp/ZZ_all/sensPlots_ZZ_linearProp/results.txt",
        "models" : ["EFTNeg"],
    ...
    plotting_nq["WZ"] = {
        "file_" : "/eos/user/g/gboldrin/www/New/linearProp/WZ_all/results.txt",
        "models" : ["EFTNeg"],
        "Color" : ROOT.kPink+9,
    ...
    plotting_nq["SSWW"] = {
        "file_" : "/eos/user/g/gboldrin/www/New/linearProp/SSWW_all/results.txt",
        "models" : ["EFTNeg"],
    ...
    plotting_nq["OSWW"] = {
        "file_" : "/eos/user/g/gboldrin/www/New/linearProp/OSWW_all/results.txt",
        "models" : ["EFTNeg"],
    ...
    # }
    plotting_nq["WW"] = {
        "file_" : "/eos/user/g/gboldrin/www/New/linearProp/inWW_all/results.txt",
        "models" : ["EFTNeg"],
    ...
    plotting_nq["Combined"] = {
        #"file_" : "/eos/user/g/gboldrin/www/New/SSWW_OSWW_OSWWQCD_inWW_WZeu_WZQCD_ZZ_noQuad/results_complete.txt",
        "file_" : "/afs/cern.ch/work/g/gboldrin/public/public/CMSSW_10_2_13/src/HiggsAnalysis/AnalyticAnomalousCoupling/newWW_noQuad/sensPlots_SSWW_OSWW_OSWWQCD_inWW_WZeu_WZQCD_ZZ_linearProp/results_complete.txt",
        "models" : ["EFTNeg"],
        "Color" : ROOT.kGray+1,
    ...
    plotting["Combined"] = {
        #"file_" : "/eos/user/g/gboldrin/www/New/SSWW_OSWW_OSWWQCD_inWW_WZeu_WZQCD_ZZ_noQuad/results_complete.txt",
        "file_" : "/afs/cern.ch/work/g/gboldrin/public/public/CMSSW_10_2_13/src/HiggsAnalysis/AnalyticAnomalousCoupling/newWW_noQuad/sensPlots_SSWW_OSWW_OSWWQCD_inWW_WZeu_WZQCD_ZZ_noQuad/results_complete.txt",
        "models" : ["EFTNeg"],
        "Color" : ROOT.kBlack,
    }
    ...
    for process in plotting.keys():
        ...
        for model in models:
            ...
            while "[MODEL RESULTS]" not in contents[i]:
                ...
                if len(line) > 1 and line[0] != "cll": #avoid empty lines at the end of file and cll
                    ops.append(line[0])
    ...
    result = all_ops[0] # HARD CODED

    result = ["cHq3", "cll1", "cHl3", "cHW", "cHq1", "cHDD", "cHWB", "cHl1", "cHbox"]
    ...
    #print(plotting_dict)
    #print(plotting_dict_nq)

    for process in plotting_dict.keys():
        for op in plotting_dict[process].keys():
            #if op in result:
            if op in ["cHq1", "cHq3", "cHWB", "cll1", "cHl1", "cHDD", "cHl3", "cHW", "cHbox"]:
                if op in plotting_dict[process].keys() and op in plotting_dict_nq[process].keys():
    ...
    first_names = gd.keys()[:4]
    #first_names = ["cHq3", "cll1", "cHl3", "cHq1"]
    first = OrderedDict()
    ...
    second_names = gd.keys()[4:]
    #second_names = ["cHDD", "cHWB", "cHl1"]
    second = OrderedDict()
    ...
    # third_names = gd.keys()[10:]
    # third = OrderedDict()
    # for name in third_names:
    #     third[name] = gd[name]

    # third_nq = OrderedDict()
    # for name in third_names:
    #     third_nq[name] = gd_q[name]
    ...
    c = ROOT.TCanvas("c", "c", 1430, 2200)
    pad2 = ROOT.TPad("pad3", "20",0.0, 0.0,1.0, 0.49)
    ...
    pad3 = ROOT.TPad("pad1", "20",0.0, 0.5,1.0, 1.0)
    #pad3 = ROOT.TPad("pad2", "20",0.0,0.6, 1.0, 1.0)
    ...
    A4 = ROOT.TGaxis(0,my, abs(mx) , my,"f4", 510, "S")
    A4.SetLabelSize(0.023)
    ...
    A5 = ROOT.TGaxis(mx ,my, 0 ,my,"f5", 510, "S")
    A5.SetLabelSize(0.023)
    ...
    leg.AddEntry(g_l, "Linear + prop. 68% C.L.", "LP")
    ...
    leg.AddEntry(g_l2, "Linear + prop. 95% C.L.", "LP")
    ...
    for i,op in enumerate(first.keys()):
        ...
        for process in first[op]:
            ...
            if op == 'cqq31' and process in ['SSWW', 'Combined', 'OSWW (EWK)']:
                g2 = first[op][process]["graph_2s"]
            ...
            #tex_1s = ROOT.TLatex(max_x + 1*max_x, first[op][process]['gy'] ,"[{:.2f}({:.2f}),{:.2f}({:.2f})]".format(first[op][process]['1s'][0], first_nq[op][process]['1s'][0], first[op][process]['1s'][1], first_nq[op][process]['1s'][1]))
            tex_1s = ROOT.TLatex(max_x + 1*max_x, first[op][process]['gy'] ,"[{:.2f}({:.2f}),{:.2f}({:.2f})]".format(first[op][process]['1s'][0], first_nq[op][process]['1s'][0], first[op][process]['1s'][1], first_nq[op][process]['1s'][1]))
            tex_1s.SetTextFont(42)
            tex_1s.SetTextSize(0.017)
            tex_1s.SetLineWidth(2)

            #tex_2s = ROOT.TLatex(max_x + 2*max_x, first[op][process]['gy'] ,"[{:.2f}({:.2f}),{:.2f}({:.2f})]".format(first[op][process]['2s'][0], first_nq[op][process]['2s'][0], first[op][process]['2s'][1], first_nq[op][process]['2s'][1]))
            tex_2s = ROOT.TLatex(max_x + 2*max_x, first[op][process]['gy'] ,"[{:.2f}({:.2f}),{:.2f}({:.2f})]".format(first[op][process]['2s'][0], first_nq[op][process]['2s'][0], first[op][process]['2s'][1], first_nq[op][process]['2s'][1]))
        ...
        my = np.mean(ys)
        tex = ROOT.TLatex(max_x + 0.5*max_x, my,"{}".format(ConvertOptoLatex(op)))
        tex.SetTextFont(52)
    ...
    f6=ROOT.TF1("f6","TMath::Power(TMath::Abs(x), [0])",0, abs(mx))
    f6.SetParameter(0, 1./2)
    A6 = ROOT.TGaxis(0,my, abs(mx) , my,"f6", 510, "S")
    A6.SetLabelSize(0.023)
    A6.SetTitleSize(0.03)
    A6.SetTitleOffset(1.2)
    A6. SetTickSize(0.09)
    A6.Draw()

    f7=ROOT.TF1("f7","-TMath::Power(TMath::Abs(x), [0])",mx,0)
    f7.SetParameter(0, 1./2)
    A7 = ROOT.TGaxis(mx ,my, 0 ,my,"f7", 510, "S")
    A7.SetLabelSize(0.023)
    A7.SetTitleSize(0.03)
    A7.SetTitleOffset(1.2)
    #A7.SetTickLength(1)
    A7. SetTickSize(0.09)
    A7.Draw()
    ...
    for op in second.keys():
        ...
        for process in second[op]:
            ...
            if op  != 'cHq1' and second[op][process]["graph_2s"].GetErrorXhigh(0) < max_x / 30 or second[op][process]["graph_2s"].GetErrorXlow(0) < min_x / 30: 
                ...
                g1.SetPointError(0, g1.GetErrorXlow(0)*25, g1.GetErrorXhigh(0)*25, g1.GetErrorYlow(0), g1.GetErrorYhigh(0)) 
                g2_f.SetPointError(0, g2_f.GetErrorXlow(0)*25, g2_f.GetErrorXhigh(0)*25, g2_f.GetErrorYlow(0), g2_f.GetErrorYhigh(0)) 
                g1_f.SetPointError(0, g1_f.GetErrorXlow(0)*25, g1_f.GetErrorXhigh(0)*25, g1_f.GetErrorYlow(0), g1_f.GetErrorYhigh(0))

            if op  != 'cHq1' and second[op][process]["graph_2s"].GetErrorXhigh(0) < max_x / 25 or second[op][process]["graph_2s"].GetErrorXlow(0) < min_x / 25: 
                name = process + " #times 10"
                g2 = second[op][process]["graph_2s"]
                g2.SetLineColor(plotting[process]["Color"])
                g2.SetMarkerColor(plotting[process]["Color"])
                g1 = second[op][process]["graph_1s"]
            ...
            if op == 'cHq1' and process in ['OSWW (EWK)', 'SSWW', 'Combined']:
                ...
                g2.SetPointError(0, g2.GetErrorXlow(0)*5, g2.GetErrorXhigh(0)*5, g2.GetErrorYlow(0), g2.GetErrorYhigh(0))
            ...
            if op == "cHl1" and process == "OSWW":
                g2.SetPointError(0, 20, 20, g2.GetErrorYlow(0), g2.GetErrorYhigh(0))
                g1.SetPointError(0, 20, 20, g1.GetErrorYlow(0), g1.GetErrorYhigh(0))
                #g2_f.SetPointError(0, g2_f.GetErrorXlow(0)*per, g2_f.GetErrorXhigh(0)*per, g2_f.GetErrorYlow(0), g2_f.GetErrorYhigh(0))
                #g1_f.SetPointError(0, g1_f.GetErrorXlow(0)*per, g1_f.GetErrorXhigh(0)*per, g1_f.GetErrorYlow(0), g1_f.GetErrorYhigh(0))
            ...
            if op == "cHW" and process in  ["ZZ", "WZ"]:
                name += " #times 0.1"
                per = 0.1
                g2.SetPointError(0, g2.GetErrorXlow(0)*per, g2.GetErrorXhigh(0)*per, g2.GetErrorYlow(0), g2.GetErrorYhigh(0))
                g1.SetPointError(0, g1.GetErrorXlow(0)*per, g1.GetErrorXhigh(0)*per, g1.GetErrorYlow(0), g1.GetErrorYhigh(0))
            ...
            if op == "cHW" and process in  ["SSWW", "OSWW", "WW", "Combined"]:
                name += " #times 0.5"
                per = 0.5
                g2.SetPointError(0, g2.GetErrorXlow(0)*per, g2.GetErrorXhigh(0)*per, g2.GetErrorYlow(0), g2.GetErrorYhigh(0))
                g1.SetPointError(0, g1.GetErrorXlow(0)*per, g1.GetErrorXhigh(0)*per, g1.GetErrorYlow(0), g1.GetErrorYhigh(0))
            ...
            print(op)
            if op == "cHl1" and process == "SSWW":
               tex_1s = ROOT.TLatex(max_x + 1*max_x, second[op][process]['gy'] ,"[>100(>100),>100(>100)]")
               tex_1s.SetTextFont(42)
               tex_1s.SetTextSize(0.017)
               tex_1s.SetLineWidth(2)

               tex_2s = ROOT.TLatex(max_x + 2*max_x, second[op][process]['gy'] ,"[>100(>100),>100(>100)]")
               tex_2s.SetTextFont(42)
               tex_2s.SetTextSize(0.017)
               tex_2s.SetLineWidth(2)
            ...
            #    tex_1s = ROOT.TLatex(max_x + 1*max_x, second[op][process]['gy'] ,"[{:.2f}({:.2f}),{:.2f}({:.2f})]".format(second[op][process]['1s'][0], second_nq[op][process]['1s'][0], second[op][process]['1s'][1], second_nq[op][process]['1s'][1]))
            #    tex_1s.SetTextFont(42)
            #    tex_1s.SetTextSize(0.023)
            #    tex_1s.SetLineWidth(2)
            
            #    #tex_2s = ROOT.TLatex(max_x + 1.75*max_x, second[op][process]['gy'] ,"[-18.5, -17.2],[{:.2f}({:.2f}),{:.2f}({:.2f})]".format(second[op][process]['2s'][0], second_nq[op][process]['2s'][0], second[op][process]['2s'][1], second_nq[op][process]['2s'][1]))
            #    tex_2s = ROOT.TLatex(max_x + 2*max_x, second[op][process]['gy'] ,"[{:.2f}({:.2f}),{:.2f}({:.2f})]".format(second[op][process]['2s'][0], second_nq[op][process]['2s'][0], second[op][process]['2s'][1], second_nq[op][process]['2s'][1]))
            #    tex_2s.SetTextFont(42)
            #    tex_2s.SetTextSize(0.023)
            #    tex_2s.SetLineWidth(2)

 
            # elif op == "cHbox" and process == "WZ (EWK)":
            #    tex_1s = ROOT.TLatex(max_x + 1*max_x, second[op][process]['gy'] ,"[{:.2f}({}),{:.2f}({})]".format(second[op][process]['1s'][0], ">100", second[op][process]['1s'][1], ">100"))
            #    tex_1s.SetTextFont(42)
            #    tex_1s.SetTextSize(0.023)
            #    tex_1s.SetLineWidth(2)

            #    tex_2s = ROOT.TLatex(max_x + 2*max_x, second[op][process]['gy'] ,"[{:.2f}({}),{:.2f}({})]".format(second[op][process]['2s'][0], ">100", second[op][process]['2s'][1], ">100"))
            #    tex_2s.SetTextFont(42)
            #    tex_2s.SetTextSize(0.023)
            #    tex_2s.SetLineWidth(2)
                ...
                # tex_1s = ROOT.TLatex(max_x + 1*max_x, second[op][process]['gy'] ,"[{:.2f}({:.2f}),{:.2f}({:.2f})]".format(second[op][process]['1s'][0], second_nq[op][process]['1s'][0], second[op][process]['1s'][1], second_nq[op][process]['1s'][1]))
                tex_1s = ROOT.TLatex(max_x + 1*max_x, second[op][process]['gy'] ,"[{:.1f}({:.1f}),{:.1f}({:.1f})]".format(second[op][process]['1s'][0], second_nq[op][process]['1s'][0], second[op][process]['1s'][1], second_nq[op][process]['1s'][1]))            
                tex_1s.SetTextFont(42)
                tex_1s.SetTextSize(0.017)
                tex_1s.SetLineWidth(2)

                # tex_2s = ROOT.TLatex(max_x + 2*max_x, second[op][process]['gy'] ,"[{:.2f}({:.2f}),{:.2f}({:.2f})]".format(second[op][process]['2s'][0], second_nq[op][process]['2s'][0], second[op][process]['2s'][1], second_nq[op][process]['2s'][1]))
                tex_2s = ROOT.TLatex(max_x + 2*max_x, second[op][process]['gy'] ,"[{:.1f}({:.1f}),{:.1f}({:.1f})]".format(second[op][process]['2s'][0], second_nq[op][process]['2s'][0], second[op][process]['2s'][1], second_nq[op][process]['2s'][1]))
                tex_2s.SetTextFont(42)
                tex_2s.SetTextSize(0.017)
                tex_2s.SetLineWidth(2)

            tex_n = ROOT.TLatex(min_x - 0.4*abs(max_x - min_x), second[op][process]['gy'] - 0.024 ,"{}".format(name))
            tex_n.SetTextFont(42)
            tex_n.SetTextSize(0.018)
            tex_n.SetLineWidth(2)
            tex_n.SetTextAlign(10)
    ...
    #         if third_nq[op][process]["2s"][1] > max_x: max_x = third_nq[op][process]["2s"][1]
    #         if third_nq[op][process]["2s"][0] < min_x: min_x = third_nq[op][process]["2s"][0] 
    ...
    # f8=ROOT.TF1("f8","TMath::Power(TMath::Abs(x), [0])",0, abs(mx))
    # f8.SetParameter(0, 1./2)
    # A8 = ROOT.TGaxis(0,my, abs(mx) , my,"f8", 505, "S")
    # A8.SetLabelSize(0.03)
    # A8.SetTitleSize(0.03)
    # A8.SetTitleOffset(1.2)
    # A8. SetTickSize(0.09)
    # A8.Draw()

    # f9=ROOT.TF1("f9","-TMath::Power(TMath::Abs(x), [0])",mx,0)
    # f9.SetParameter(0, 1./2)
    # A9 = ROOT.TGaxis(mx ,my, 0 ,my,"f9", 505, "S")
    # A9.SetLabelSize(0.03)
    # A9.SetTitleSize(0.03)
    # A9.SetTitleOffset(1.2)
    # #A9.SetTickLength(1)
    # A9. SetTickSize(0.09)
    # A9.Draw()
    ...
    #         if op == "cll" and process in ["WZ", "SSWW",  "OSWW", "Combined"]:
    ...
    #             g2_extended = deepcopy(g2)
    #             x = g2_extended.GetX()
    #             x[0] = mt.sqrt(16.12382028156308)*per 
    #             x2 = mt.sqrt(21.713)*per
    #             g2_extended.SetPointError(0, 0, x2-x[0], g2.GetErrorYlow(0), g2.GetErrorYhigh(0))
    #             g2_extended.SetMarkerSize(0)
    ...
    #         if op == "cHbox" and process == "ZZ":
    #            tex_1s = ROOT.TLatex(max_x + 1*max_x, third[op][process]['gy'] ,"[{:.2f}({}),{:.2f}({})]".format(third[op][process]['1s'][0], ">100", third[op][process]['1s'][1], ">100"))
    #            tex_1s.SetTextFont(42)
    #            tex_1s.SetTextSize(0.023)
    #            tex_1s.SetLineWidth(2)

    #            tex_2s = ROOT.TLatex(max_x + 2*max_x, third[op][process]['gy'] ,"[{:.2f}({}),{:.2f}({})]".format(third[op][process]['2s'][0], ">100", third[op][process]['2s'][1], ">100"))
    #            tex_2s.SetTextFont(42)
    #            tex_2s.SetTextSize(0.023)
    #            tex_2s.SetLineWidth(2)
    ...
    #            tex_1s = ROOT.TLatex(max_x + 1*max_x, third[op][process]['gy'] ,"[{:.1f}({}),{:.1f}({})]".format(third[op][process]['1s'][0], ">100", third[op][process]['1s'][1], ">100"))
    #            tex_1s.SetTextFont(42)
    #            tex_1s.SetTextSize(0.023)
    #            tex_1s.SetLineWidth(2)

    #            tex_2s = ROOT.TLatex(max_x + 2*max_x, third[op][process]['gy'] ,"[{:.1f}({}),{:.1f}({})]".format(third[op][process]['2s'][0], ">100", third[op][process]['2s'][1], ">100"))
    #            tex_2s.SetTextFont(42)
    #            tex_2s.SetTextSize(0.023)
    #            tex_2s.SetLineWidth(2)

    #         elif op == "cll" and process == "ZZ":
    #            tex_1s = ROOT.TLatex(max_x + 1*max_x, third[op][process]['gy'] ,"[{}({}),{}({})]".format(">100", ">100", ">100", ">100"))
    #            tex_1s.SetTextFont(42)
    #            tex_1s.SetTextSize(0.023)
    #            tex_1s.SetLineWidth(2)

    #            tex_2s = ROOT.TLatex(max_x + 2*max_x, third[op][process]['gy'] ,"[{}({}),{}({})]".format(">100", ">100", ">100", ">100"))
    #            tex_2s.SetTextFont(42)
    #            tex_2s.SetTextSize(0.023)
    #            tex_2s.SetLineWidth(2)
    ...
    #            tex_1s = ROOT.TLatex(max_x + 1*max_x, third[op][process]['gy'] ,"[{:.2f}({:.2f}),{:.2f}({:.2f})]".format(third[op][process]['1s'][0], third_nq[op][process]['1s'][0], third[op][process]['1s'][1], third_nq[op][process]['1s'][1]))
    #            tex_1s.SetTextFont(42)
    #            tex_1s.SetTextSize(0.023)
    #            tex_1s.SetLineWidth(2)
    ...
    #            tex_2s = ROOT.TLatex(max_x + 2*max_x, third[op][process]['gy'] ,"[{:.2f}({:.2f}),{:.2f}({:.2f})]".format(third[op][process]['2s'][0], third_nq[op][process]['2s'][0], third[op][process]['2s'][1], third_nq[op][process]['2s'][1]))
    #            tex_2s.SetTextFont(42)
    #            tex_2s.SetTextSize(0.023)
    #            tex_2s.SetLineWidth(2)
    ...
    #         tex_n.SetTextFont(42)
    #         tex_n.SetTextSize(0.025)
    #         tex_n.SetLineWidth(2)
    #         tex_n.SetTextAlign(10)

    #         limits3.append(tex_1s)
    #         limits3.append(tex_2s)
    #         name_proc_3.append(tex_n)
    ...
    tex3 = ROOT.TLatex(0.87,.96,"#Lambda=1 TeV   100 fb^{-1}   (13 TeV)")
    tex3.SetNDC()
    ...
    tex4 = ROOT.TLatex(0.479,.03, "Parameter Estimate [TeV^{-2}]")
    tex4.SetNDC()
    ...
    tex5 = ROOT.TLatex(0.738,.86, "68% L (L + prop.)")
    tex5.SetNDC()
    tex5.SetTextAlign(31)
    tex5.SetTextFont(42)
    tex5.SetTextSize(0.015)
    tex5.SetLineWidth(2)
    tex5.Draw()

    #tex6 = ROOT.TLatex(0.85,.79, "2#sigma")
    tex6 = ROOT.TLatex(0.88,.86, "95% L+Q (L + prop.)")
    tex6.SetNDC()
    tex6.SetTextAlign(31)
    tex6.SetTextFont(42)
    tex6.SetTextSize(0.015)
    tex6.SetLineWidth(2)
    tex6.Draw()


    c.Draw()
    c.Print("summary_linearProp.pdf")
